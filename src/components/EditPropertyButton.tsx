...
import { updateProperty } from "@/app/admin/clients/actions";
...
export function EditPropertyButton({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await updateProperty(propertyId, name, address || null);
        setOpen(false);
        router.refresh();
      } catch (err) {
  ...
  return (
    <>
      <button
        type="button"
        onClick={openEditor}
        aria-label="Edit property name and address"
      ...
      {open && (
        <Modal title="Edit property" onClose={() => setOpen(false)}>
